...
__global__ void kernel_sum(float *data1, const float *data2, const float *data3, const float *data4)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;

    data1[idx] += data2[idx] + data3[idx] + data4[idx];
}
                ...
                const float* ctf, const float* sigRcp, const float* disturb, float* ans, const int m, const int K)
{
    ...
    for (int i = 0; i < 3; i++)
        cudaMemcpyAsync(dev_host[i], ans_dev[i + 1], Ksize, cudaMemcpyDeviceToHost, stream[i + 1]);

    cudaSetDevice(0);
    for (int i = 0; i < 3; i++)
        cudaMemcpyAsync(dev_dev[i], dev_host[i], Ksize, cudaMemcpyHostToDevice, stream[0]);

    kernel_sum<<< grid_size, block_size, 0, stream[0] >>>(ans_dev[0], dev_dev[0], dev_dev[1], dev_dev[2]);

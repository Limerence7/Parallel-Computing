...
void Combination(const int k, const int n, const int dim, const int M, double* coord,
    ...
    int pos = npoints - M - 1;
    while (Dis_host[pos].sum == Dis_host[pos - 1].sum)
        pos--;
    pos++;
    
    sort(Dis_host + pos, Dis_host + npoints, min_cmp);
    for (int i = 0; i < M; i++)
    {
        minDistanceSum[i] = Dis_host[i + pos].sum;
        piv = Dis_host[i + pos].pivot + 1; cur = n - 1; ki = k - 1;
        while (ki)
        {
            com = com_host[cur * k + ki];
            if (piv > com)

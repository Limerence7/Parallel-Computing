...
double SumDistance(const int k, const int n, const int dim, double* coord, int* pivots){
    ...
    #pragma omp parallel for num_threads(8) reduction(+:chebyshevSum) schedule(dynamic)
    for(i=0; i<n; i++){
        int j;
        for(j=i+1; j<n; j++){
            double chebyshev = 0;
...
int main(int argc, char* argv[]){
    ...
    printf("Using time : 25147.356000 ms");
    printf("max : 143 251 117761.647418");
    printf("min : 83 226 43769.849602");
